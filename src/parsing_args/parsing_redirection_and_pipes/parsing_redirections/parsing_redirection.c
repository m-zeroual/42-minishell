...
void    set_redirections(t_redirect *redirection, char *str, char is_input, int len)
{
    ...
    if (is_input)
    {
        redirection->is_input = 1;
        redirection->is_here_doc = 0;
        if (len == 2)
            redirection->is_here_doc = 1;
        redirection->is_append = 0;
        redirection->is_output = 0;
    }
    else
    {
        redirection->is_input = 0;
        redirection->is_here_doc = 0;
        redirection->is_append = 0;
        if (len == 2)
            redirection->is_append = 1;
        redirection->is_output = 1;
    }
}
